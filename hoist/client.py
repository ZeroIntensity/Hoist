...
class Client:
    @staticmethod
    def add_hoist(app: Flask, handle_errors: bool = True) -> Flask:
    ...
    def create_server(self, ip: str = "", port: int = 0, logging: bool = False, startup_message: bool = False, thread: bool = True, run = True, handle_errors: bool = True) -> HoistServer: # Function for creating flask app with hoist route
        ...
        wrapper.add_hoist(app, handle_errors) # Add hoist to flask app
        if run:
            if thread:
